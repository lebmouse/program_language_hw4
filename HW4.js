...
class Assistant {
    ...
    getTotalPrice() {
        let customer = this.customer, orders = customer.getOrders(), totalPrice = 0;
        for (let i = 0; i < customer.getOrders().length; i++) {
            let drink = orders[i].getDrink(), drinkPrice = drink.getPrice(), drinkNumber = orders[i].getNumber();
            totalPrice += drinkPrice * drinkNumber;
        }
        return totalPrice * customer.getDiscount();
    }
    getTotalTime() {
        let customer = this.customer, orders = customer.getOrders(), totalTime = 0;
        for (let i = 0; i < customer.getOrders().length; i++) {
            let drink = orders[i].getDrink(), drinkTime = drink.getTime(), drinkNumber = orders[i].getNumber();
